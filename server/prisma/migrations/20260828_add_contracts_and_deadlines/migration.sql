...
CREATE TABLE "request_deadline_settings" (
    "id" TEXT NOT NULL,
...
-- Новые поля в tm_objects
ALTER TABLE "tm_objects" ADD COLUMN "contract_id" TEXT;

-- Новые поля в imported_requests
...
-- Новые поля в mtr_tm_objects
ALTER TABLE "mtr_tm_objects" ADD COLUMN "contract_id" TEXT;

-- Новое поле в mtr_visits
ALTER TABLE "mtr_visits" ADD COLUMN "contract_id" TEXT;

-- Индексы
CREATE INDEX "contracts_tm_id_idx" ON "contracts"("tm_id");
CREATE INDEX "contracts_number_idx" ON "contracts"("number");
CREATE UNIQUE INDEX "contracts_number_tm_id_module_key" ON "contracts"("number", "tm_id", "module");

CREATE UNIQUE INDEX "request_deadline_settings_request_type_key" ON "request_deadline_settings"("request_type");

CREATE INDEX "tm_objects_contract_id_idx" ON "tm_objects"("contract_id");
CREATE INDEX "imported_requests_contract_id_idx" ON "imported_requests"("contract_id");
CREATE INDEX "visits_contract_id_idx" ON "visits"("contract_id");
CREATE INDEX "mtr_tm_objects_contract_id_idx" ON "mtr_tm_objects"("contract_id");
CREATE INDEX "mtr_visits_contract_id_idx" ON "mtr_visits"("contract_id");
...
-- Seed настроек сроков по умолчанию
INSERT INTO "request_deadline_settings" ("id", "request_type", "deadline_days", "notification_days_before", "updated_by", "updated_at")
VALUES 
    (gen_random_uuid()::text, 'planned'::"RequestType", NULL, 5, (SELECT id FROM users WHERE role = 'admin' LIMIT 1), NOW()),
    (gen_random_uuid()::text, 'unplanned'::"RequestType", 14, 5, (SELECT id FROM users WHERE role = 'admin' LIMIT 1), NOW());

-- Seed договоров для существующих ТМ
INSERT INTO "contracts" ("id", "number", "tm_id", "module", "is_active", "created_at")
VALUES 
    (gen_random_uuid()::text, '050005596590', 'b9758bf8-cf33-4d13-a741-edbb4d55748f', 'to', true, NOW()),
    (gen_random_uuid()::text, '050001234567', '955f520b-41f4-4236-9ecf-e11365c81824', 'to', true, NOW()),
    (gen_random_uuid()::text, '050007654321', '8d0d86bc-ff3c-4c89-9383-530ed8e044f0', 'to', true, NOW()),
    (gen_random_uuid()::text, '050001234567', '7b203951-263f-442d-8d0a-2bc73ea17745', 'mtr', true, NOW());
